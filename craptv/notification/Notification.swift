...
extension AppDelegate {
  func observe() {
    ...
    var hasSidebar: Bool {
      ...
      set {
        _hasSidebar = newValue || _hasSidebar
      }
    }
    ...
    center.addObserver(forName: .toggleFullscreen, object: nil, queue: mainQueue) { _ in
      ...
      guard let psize = self.player.size else {
        return
      }

      self.window.aspectRatio = psize

      guard self.window.isFullScreen else {
        return
      }

      guard player.contentToggle == nil else {
        return
      }
    }
    ...
    center.addObserver(forName: .startPlaying, object: nil, queue: mainQueue) { note in
      ...
      guard stream.is_adult == false else {
        return
      }
      Task.init {
        do {
