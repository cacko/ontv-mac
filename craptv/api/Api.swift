...
enum API {
  ...
  class ApiAdapter: NSObject, ObservableObject {
    ...
    @Published var epgState: EPGState = .notavail
    ...
    func login() {
      ...
      if username.count == 0 || password.count == 0 {
        state = .error
        error = API.Exception.invalidLogin("new app")
...
//            || min(self.count(Stream.self), self.count(Category.self)) == 0
          {
...
//              || self.count(EPG.self) == 0
    ...
    func updateUser() async throws {
      do {
        let json = try await fetchData(url: Endpoint.Login)
