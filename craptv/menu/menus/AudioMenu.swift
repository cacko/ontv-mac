...
class AudioItem: NoModifierItem {
  var parameter: Audio.Parameter

  init(title: String, action: Selector?, keyEquivalent: String, parameter: Audio.Parameter) {
...
class AudioLabelItem: BaseItem {

  let command: Audio.Command
  let prefix: String

  init(
    title: String, action: Selector?, keyEquivalent: String, audioCommand: Audio.Command
  ) {
    self.prefix = title
    self.command = audioCommand

    super.init(title: "\(title) \(Player.instance.volume)", action: action, keyEquivalent: keyEquivalent)
...
class AudioMenu: BaseMenu {
  override var actions: [NSMenuItem] {
    [
      NoModifierItem(
        title: "Toggle sound", action: #selector(onAudioMute(sender:)), keyEquivalent: "m"),
      AudioItem(
        title: "Gain down", action: #selector(onAudioCommand(sender:)), keyEquivalent: "[",
        parameter: Audio.Parameter(command: .volume_offset, value: -5)),
      AudioItem(
        title: "Gain up", action: #selector(onAudioCommand(sender:)), keyEquivalent: "]",
        parameter: Audio.Parameter(command: .volume_offset, value: 5)),
      AudioLabelItem(
        title: "Volume:", action: nil, keyEquivalent: "",
